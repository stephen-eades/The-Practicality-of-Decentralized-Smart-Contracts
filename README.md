...
## Synopsis
For this project I will build a decentralized application (dApp) that can be used to create and deploy customizable smart contracts. The user can access the dApp from any web browser, and will be able to choose a contract template (poll, lottery, or escrow). They can input parameters for the smart contract to implement before being deployed, such as how long the contract should be active for. Once deployed, any other user can use the dApp to view and interact with the smart contract, for example if they wanted to vote in a poll.
...
* Are smart contracts a practical solution for handling processes between users, such as voting, escrow, and lotterys?
